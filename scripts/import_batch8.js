/**
 * Import Batch 8 LOGIMAT Leads - PRODUCTION
 */
const fs = require('fs');
...
async function importBatch8() {
    try {
        ...
        const jsonPath = path.join(__dirname, '..', 'LEADS', 'extracted_leads_batch8.json');
        const leadsData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
        console.log(`📋 Found ${leadsData.length} LOGIMAT BATCH 8 leads to import...\n`);
        ...
        console.log(`\n========================================`);
        console.log(`📊 LOGIMAT BATCH 8 IMPORT COMPLETE`);
