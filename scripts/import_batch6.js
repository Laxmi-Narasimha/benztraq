/**
 * Import Batch 6 LOGIMAT Leads - PRODUCTION
 */
const fs = require('fs');
...
async function importBatch6() {
    try {
        ...
        const jsonPath = path.join(__dirname, '..', 'LEADS', 'extracted_leads_batch6.json');
        const leadsData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
        console.log(`📋 Found ${leadsData.length} LOGIMAT BATCH 6 leads to import...\n`);
        ...
        console.log(`\n========================================`);
        console.log(`📊 LOGIMAT BATCH 6 IMPORT COMPLETE`);
