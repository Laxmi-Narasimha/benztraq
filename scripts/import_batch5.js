/**
 * Import Batch 5 LOGIMAT Leads - PRODUCTION
 */
const fs = require('fs');
...
async function importBatch5() {
    try {
        ...
        const jsonPath = path.join(__dirname, '..', 'LEADS', 'extracted_leads_batch5.json');
        const leadsData = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
        console.log(`📋 Found ${leadsData.length} LOGIMAT BATCH 5 leads to import...\n`);
        ...
        console.log(`\n========================================`);
        console.log(`📊 LOGIMAT BATCH 5 IMPORT COMPLETE`);
